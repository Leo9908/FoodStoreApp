    ...
    async delete(id, place) {
      try {
        ...
        if (place == true) {
          ...
          const deleted = this.getOnSaleProductById(id);
          const index = this.onSaleDishes.indexOf(deleted);
          const index2 = this.favoriteDishes.indexOf(deleted);
          if (index > 0) this.onSaleDishes.splice(index, 1);
          if (index2 > 0) this.favoriteDishes.splice(index2, 1);
          this.deleteImage(deleted.imgName);
        }
      } catch (error) {
        Notify.create({
